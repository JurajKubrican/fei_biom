...
def find_contours(gray):
    ...
    canny = cv2.Canny(gray, 50, 80, apertureSize=3, L2gradient=False)
    print(canny.shape)
    ret, canny = cv2.threshold(gray, 150, 255, 0)
    print(canny.shape)

    img, cnt, hierarchy = cv2.findContours(image=canny,
                                           mode=cv2.RETR_TREE,
    ...
    ellipse_img = cv2.ellipse(img, ellipse, color=(0, 255, 0))
